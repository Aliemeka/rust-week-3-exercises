...
impl CompactSize {
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        ...
        match self.value {
            ...
            _ => {
                ...
                bytes.extend_from_slice(&self.value.to_le_bytes()); // By default, value is u64
                bytes
            }
        }
    }
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<(Self, usize), BitcoinError> {
        ...
        match first_byte {
            0x00..=0xFC => Ok((Self::new(first_byte as u64), 1)),
            0xFD => {
                if bytes.len() < 3 {
                    return Err(BitcoinError::InsufficientBytes);
                }
...
impl Serialize for Txid {
        ...
        S: serde::Serializer,
    {
        ...
        let encoded_string = hex::encode(self.0);
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        if bytes.len() != 32 {
            return Err(deserializeError::custom(format!(
                "Expected 32 bytes for Txid, got {} bytes",
                bytes.len()
            )));
        }
...
impl OutPoint {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<(Self, usize), BitcoinError> {
        ...
        Ok((Self::new(txid_array, vout), 36))
    }
}
...
impl Script {
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        // Prefix with CompactSize (length), then raw bytes// Referencing here to avoid cloning
        let bytes_length = self.bytes.len() as u64;
        let length_prefix = CompactSize::new(bytes_length).to_bytes();
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<(Self, usize), BitcoinError> {
        ...
        Ok((Self::new(script_bytes), total_size))
    }
}
...
impl TransactionInput {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<(Self, usize), BitcoinError> {
        ...
        sequence_bytes.copy_from_slice(&bytes[offset..(4 + offset)]);
        let sequence = u32::from_le_bytes(sequence_bytes);
        offset += 4;
        Ok((Self::new(previous_output, script_sig, sequence), offset))
    }
}
...
impl BitcoinTransaction {
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        ...
        bytes.extend_from_slice(&CompactSize::new(input_count).to_bytes());
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<(Self, usize), BitcoinError> {
        ...
        let (input_count, cs_size) = CompactSize::from_bytes(&bytes[4..])?;
        ...
        lock_time_bytes.copy_from_slice(&bytes[offset..(4 + offset)]);
...
impl fmt::Display for OutPoint {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "OutPoint {{ Txid: {}, Vout: {} }}", self.txid, self.vout)
    }
}

impl fmt::Display for Script {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "Script {{ Bytes: {} }}", hex::encode(&self.bytes))
    }
}
...
impl fmt::Display for TransactionInput {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "TransactionInput {{ Previous Output: {}, Script Sig: {}, Sequence: {} }}",
            self.previous_output, self.script_sig, self.sequence
        )
    }
}
...
impl fmt::Display for BitcoinTransaction {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        write!(
            f,
            "BitcoinTransaction {{ Version: {}, Inputs: [",
            self.version
        )?;
        for input in &self.inputs {
            write!(
                f,
                "{{ Previous Output Vout: {}, Script Sig: {}, Sequence: {} }}, ",
                input.previous_output.vout, input.script_sig, input.sequence
            )?;
        }
